...
// Posição x (borda direita) das colunas numéricas do layout LEGADO: COLS[i].x + w − 1.
...
/** Linhas (dados + TOTAIS) com as 3 colunas numéricas lidas de volta em centavos (layout legado). */
...
function row(over: Partial<ContractorReportRow>): ContractorReportRow {
  return {
    ...
    // Layout legado (sem decomposição) — as vagas destes testes são anteriores
    // ao modelo novo (spec 2026-09-02).
    taxa_servico_in_cents: null,
    seguro_in_cents: null,
    total_freelance_in_cents: null,
    inss_in_cents: null,
